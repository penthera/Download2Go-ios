...
@property (nonatomic, assign) Boolean refreshInProcess;
...
- (IBAction)downloadBtnClicked:(UIButton *)sender
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        [VirtuosoSettings instanceOnReady:^(VirtuosoSettings * _Nonnull instance) {
            instance.audioLanguagesToDownload = [NSArray arrayWithObject:@"sp"];
        }];
        
        // Important:
        // AssetID should be unique across your video catalog
        NSString* myAssetID = @"SINGLE_CC_TEST";
        
        // Important:
        // Create asset configuration object
        VirtuosoAssetConfig* config = [[VirtuosoAssetConfig alloc]initWithURL:@"https://playertest.longtailvideo.com/adaptive/eleph-audio/playlist.m3u8"
                                                                      assetID:myAssetID
                                                                  description:@"Test Asset" type:kVDE_AssetTypeHLS];
        if (!config)
        {
...
{
    [self reparseManifestAndRefreshExample];
}
...
-(void)reparseManifestAndRefreshExample
{
    // Important:
    // Refresh the asset and select ALL audio languages. This will cause
    // new languages to be parsed, downloaded and added to the existing download.
    ...
    // Reparse will include all languages
    [VirtuosoSettings instanceOnReady:^(VirtuosoSettings * _Nonnull instance) {
        instance.audioLanguagesToDownload = nil;
    }];
    
    self.refreshInProcess = true;
    self.statusLabel.text = [NSString stringWithFormat:@"Starting asset refresh."];
    
    // Refresh manfiest and start downloading content
    [self.exampleAsset refreshManifestAndDownload:^(VirtuosoAsset *parsedAsset, NSError *error) {
        NSLog(@"Manifest has been refreshed, download will follow immediately.");
    }];
}
...
-(void)refreshView
{
    if (nil == self.exampleAsset) {
        ...
        [self setEnabledAppearance:self.refreshButton enabled:NO];
    ...
    {
        self.statusLabel.text = @"Refresh downloading...";
        
        [self setEnabledAppearance:self.deleteBtn enabled:NO];
    ...
    {
        [self setEnabledAppearance:self.refreshButton enabled:YES];
...
-(void)downloadEngineDidFinishRefreshingAsset:(VirtuosoAsset*)asset {
    
    // Important:
    // This delegate method is invoked when download completes on the refreshed asset.
    
    NSLog(@"Download finished following asset refresh. Now safe to play.");
    if ([asset.uuid isEqualToString:self.exampleAsset.uuid])
    {
        self.exampleAsset = asset;
